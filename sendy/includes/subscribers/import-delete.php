...
if(!file_exists($csvfile)) {
	header("Location: ".get_app_info('path').'/delete-from-list?i='.$app.'&l='.$listID.'&e=3'); 
	exit;
}
...
foreach(explode($lineseparator,$csvcontent) as $line)
{
	...
	if($columns>1)
	{
		header("Location: ".get_app_info('path').'/delete-from-list?i='.$app.'&l='.$listID.'&e=1'); 
		exit;
	}
	...
	if (mysqli_num_rows($r) > 0)
	{
		...
		$query = 'DELETE FROM '.$databasetable.' WHERE email = "'.$line.'" AND list = '.$listID.' AND userID = '.$userID;
