...
$q = "SHOW COLUMNS FROM login WHERE Field = 'db_updated'";
$r = mysqli_query($mysqli, $q);
if (!$r || mysqli_num_rows($r) == 0)
{
	$r2 = mysqli_query($mysqli, 'ALTER TABLE login ADD db_updated VARCHAR(100)');
	if(!$r2) error_log("[Can\'t run ALTER TABLE login ADD db_updated VARCHAR(100)]".mysqli_error($mysqli).': in '.__FILE__.' on line '.__LINE__);
}
...
//=================================================//
$q = 'SELECT db_updated FROM login WHERE (db_updated != "'.CURRENT_VERSION.'" OR db_updated is NULL) AND license != ""';
$r = mysqli_query($mysqli, $q);
...
{	
	...
	if (!$r || mysqli_num_rows($r) == 0)
	{
		...
		$month_today = date("m", $time_today_unix);
		$year_today = date("Y", $time_today_unix);
	...
	//=================================================//
	//Create new columns in 'lists' table
	$q = 'SHOW TABLES LIKE "suppression_list"';
	$r = mysqli_query($mysqli, $q);
	if (mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, '
	...
	//================== Version 5.2.1 ==================//
	//New index in table: links
	//=================================================//
	
	//add index in links table
	$q = 'SHOW INDEX FROM links WHERE KEY_NAME = "s_campaign_id"';
	$r = mysqli_query($mysqli, $q);
	if (!$r || mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, 'CREATE INDEX s_campaign_id ON links (campaign_id)');
		mysqli_query($mysqli, 'CREATE INDEX s_ares_emails_id ON links (ares_emails_id)');
		mysqli_query($mysqli, 'ALTER TABLE apps ADD COLUMN opens_tracking INT (1) DEFAULT \'1\', ADD COLUMN links_tracking INT (1) DEFAULT \'1\'');
	}
	
	//================== Version 5.2.5 ==================//
	//Change row_format of 'apps' table to 'Dynamic'
	//=================================================//
	
	//Check row_format of 'apps' table
	$q = "SELECT row_format FROM information_schema.tables WHERE table_name='apps' LIMIT 1";
	$r = mysqli_query($mysqli, $q);
	if ($r && mysqli_num_rows($r) > 0)
		while($row = mysqli_fetch_array($r))
			$apps_row_format = $row['row_format'];
	
	if($apps_row_format != 'Dynamic')
	{
		mysqli_query($mysqli, 'ALTER TABLE apps ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE ares ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE ares_emails ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE blocked_domains ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE campaigns ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE links ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE lists ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE login ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE queue ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE rules ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE seg ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE seg_cons ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE skipped_emails ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE subscribers ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE subscribers_seg ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE suppression_list ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE template ROW_FORMAT=DYNAMIC');
		mysqli_query($mysqli, 'ALTER TABLE zapier ROW_FORMAT=DYNAMIC');
	}
	
	//================== Version 6 ==================//
	//New column in table: campaigns
	//=================================================//
	
	//Add column web_version_lang in campaigns table
	$q = "SHOW COLUMNS FROM campaigns WHERE Field = 'web_version_lang'";
	$r = mysqli_query($mysqli, $q);
	if (!$r || mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, 'ALTER TABLE campaigns ADD web_version_lang VARCHAR(50)');
		mysqli_query($mysqli, 'ALTER TABLE rules MODIFY COLUMN endpoint VARCHAR(255);');
		mysqli_query($mysqli, 'ALTER TABLE lists ADD hide INT(1) DEFAULT 0');
		mysqli_query($mysqli, 'ALTER TABLE apps ADD hide_lists INT(1) DEFAULT 1');
	}
	
	//================== Version 6.0.2 ==================//
	//New column in table: campaign_stopped
	//=================================================//
	
	//Add column web_version_lang in campaigns table
	$q = "SHOW COLUMNS FROM campaigns WHERE Field = 'campaign_stopped'";
	$r = mysqli_query($mysqli, $q);
	if (!$r || mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, 'ALTER TABLE campaigns ADD campaign_stopped INT(1) DEFAULT 0');
	}
	
	//================= Version 6.0.5 ===============//
	//New column in table: rules
	//=================================================//
	//Create new 'allowed_attachments' in 'apps' table
	$q = "SHOW COLUMNS FROM rules WHERE Field = 'enabled'";
	$r = mysqli_query($mysqli, $q);
	if (mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, 'ALTER TABLE rules ADD COLUMN enabled INT (11) DEFAULT \'0\'');
		mysqli_query($mysqli, 'UPDATE rules SET enabled = 1');
	}
	...
	//=================================================//
	//Create new 'allowed_attachments' in 'apps' table
	$q = "SHOW COLUMNS FROM template WHERE Field = 'from_name'";
	$r = mysqli_query($mysqli, $q);
	if (mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, 'ALTER TABLE template ADD COLUMN from_name VARCHAR (100)');
		mysqli_query($mysqli, 'ALTER TABLE template ADD COLUMN from_email VARCHAR (100)');
		mysqli_query($mysqli, 'ALTER TABLE template ADD COLUMN reply_to VARCHAR (100)');
		mysqli_query($mysqli, 'ALTER TABLE apps ADD COLUMN opt_in INT (1) DEFAULT 0');
	}
	
	//================== Version 6.0.7 ==================//
	//New column in table: ignore_checks
	//=================================================//
	
	//Add column web_version_lang in campaigns table
	$q = "SHOW COLUMNS FROM campaigns WHERE Field = 'ignore_checks'";
	$r = mysqli_query($mysqli, $q);
	if (!$r || mysqli_num_rows($r) == 0)
	{
		mysqli_query($mysqli, 'ALTER TABLE campaigns ADD ignore_checks INT(1) DEFAULT 1');
	}
	
	//================== Version 6.0.7.1 ==================//
	//Set default for ignore_checks to 1
	//=================================================//
	
	//Set default for ignore_checks to 1
	mysqli_query($mysqli, 'ALTER TABLE campaigns ALTER ignore_checks SET DEFAULT 1');
	
	//=================================================//
	//Don't run this script again
	//=================================================//
	$q = 'UPDATE login SET db_updated = "'.CURRENT_VERSION.'" WHERE id = 1';
	mysqli_query($mysqli, $q);
}
?>