...
<?php 
	...
	foreach ($conditions_array as $and_group)
	{
		...
		foreach($and_group as $or_group)
		{
			...
			$q = 'INSERT INTO seg_cons (seg_id, group_id, operator, field, comparison, val) VALUES ('.$sid.', '.$i.', "'.$operator.'", "'.$field.'", "'.$comparison.'", "'.$val.'")';
			$r = mysqli_query($mysqli, $q);
			if (!$r)
			{
				error_log("[Can't INSERT row in seg_cons table]".mysqli_error($mysqli).': in '.__FILE__.' on line '.__LINE__);
