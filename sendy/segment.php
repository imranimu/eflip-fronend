...
<div class="row-fluid">
    ...
    <div class="span10" id="holder">
	    ...
	    <form action="<?php echo get_app_info('path')?>/includes/segments/save.php" method="POST" accept-charset="utf-8" class="form-vertical" id="seg-form">
						...
						<div class="or-group" style="display:none;">
							<div class="dropdown condition-dropdown">
								...
								<ul class="dropdown-menu">
									...
									<li><a href="javascript:void(0)" class="text-fields" data-field="country"><i class="icon icon-globe"></i> <?php echo _('Country');?></a></li>
									<li><a href="javascript:void(0)" class="text-fields" data-field="referrer"><i class="icon icon-share-alt"></i> <?php echo _('Referrer');?></a></li>
									<?php if($gdpr_options):?>
									<li><a href="javascript:void(0)" class="boolean-fields" data-field="gdpr"><i class="icon icon-certificate"></i> <?php echo _('GDPR');?></a></li>
									<?php endif;?>
									<br/>
									<?php 
					        ...
					        <div class="boolean-comparisons" style="display:none;">
								<div class="dropdown boolean-condition-dropdown">
									<button class="btn dropdown-toggle" type="button" data-toggle="dropdown"><span class="dropdown-label" data-selected="="><?php echo _('is tagged');?></span>
									<span class="caret"></span></button>
									<ul class="dropdown-menu">
										<li><a href="javascript:void(0)" data-field="="><?php echo _('is tagged');?></a></li>
										<li><a href="javascript:void(0)" data-field="!="><?php echo _('is not tagged');?></a></li>
									</ul>
								</div>
					        </div>
		    ...
		    <input type="hidden" id="app" name="app" value="<?php echo get_app_info('app');?>" />
	    ...
	    <script type="text/javascript">
		    $(document).ready(function() {
				...
				<?php 
				    ...
				    if(!$new)
				    {
					    $default_fields = array('join_date', 'timestamp', 'name', 'email', 'country', 'referrer', 'gdpr');								
					    $icons = array($default_fields[0]=>'expand-alt', $default_fields[1]=>'time', $default_fields[2]=>'user', $default_fields[3]=>'envelope-alt', $default_fields[4]=>'globe', $default_fields[5]=>'share-alt');
					    $field_text_array = array($default_fields[0]=>_('Signed up'), $default_fields[1]=>_('Last activity'), $default_fields[2]=>_('Name'), $default_fields[3]=>_('Email address'), $default_fields[4]=>_('Country'), $default_fields[5]=>_('Referrer'));
					    $field_type_array = array($default_fields[0]=>'date-fields', $default_fields[1]=>'date-fields', $default_fields[2]=>'text-fields', $default_fields[3]=>'text-fields', $default_fields[4]=>'text-fields', $default_fields[5]=>'text-fields');	
					    
					    if($gdpr_options)
					    {
						    array_push($default_fields, 'gdpr');
						    $icons[$default_fields[6]] = 'certificate';
						    $field_text_array[$default_fields[6]] = _('GDPR');
						    $field_type_array[$default_fields[6]] = 'boolean-fields';
					    }
					    ...
					    $q = 'SELECT id, group_id, operator, field, comparison, val FROM seg_cons WHERE seg_id = '.$sid;
					    $r = mysqli_query($mysqli, $q);
	    		...
	    		?>
						    		...
						    		$prev_group = $("#group<?php echo $group_id-1;?>");
									...
									$new_or.find(".boolean-comparisons").hide();
									...
									<?php if($field_type=='date-fields'):?>
										...
										<?php 
											if($comparison=='BETWEEN'):
												...
												$year1 = date('Y', $c_a[0]);
												$year2 = date('Y', $c_a[1]);
												$start_date = date("D M d $year1", $c_a[0]);
												$end_date = date("D M d $year2", $c_a[1]);
										?>
										...
										<?php else:?>
											<?php $year = date('Y', $val);?>
											$new_or.find(".date1 .datepicker1").val("<?php echo date("D M d $year", $val);?>");
										<?php endif;?>
									...
									<?php elseif($field_type=='boolean-fields'):?>								
										$new_or.find(".boolean-comparisons").show();
										$new_or.find(".boolean-condition-dropdown button span.dropdown-label").text("<?php echo $boolean_comparisons_text[$comparison]?>");
										$new_or.find(".boolean-condition-dropdown button span.dropdown-label").attr("data-selected", "<?php echo $comparison;?>");
									<?php endif;?>
	    		...
	    		?>
									...
									<?php elseif($field_type=='boolean-fields'):?>
										...
										$new_or.find(".boolean-condition-dropdown button span.dropdown-label").text("<?php echo $boolean_comparisons_text[$comparison]?>");
										$new_or.find(".boolean-condition-dropdown button span.dropdown-label").attr("data-selected", "<?php echo $comparison;?>");
										$new_or.find(".boolean-comparisons .text-value").val("<?php $val = ltrim($val, '%'); $val = rtrim($val, '%'); echo $val;?>");
									<?php endif;?>
									$new_or.show();
									$new_or.fadeIn();
				...
				function cloneORCondition(e)
				{
					...
					$new_or.find(".boolean-comparisons button span.dropdown-label").attr("data-selected", "=");
					$new_or.find(".text-comparisons button span.dropdown-label").text("is");
					$new_or.find(".boolean-comparisons button span.dropdown-label").text("is tagged");
					$new_or.find(".date-comparisons button span.dropdown-label").text("on");
					$new_or.find(".text-comparisons input, .date-comparisons input, .boolean-comparisons input").val("");
					$new_or.find(".text-comparisons input, .date-comparisons input, .boolean-comparisons input").css("border-color", "#<?php echo $border_color;?>");
					$new_or.find(".condition-dropdown button").css("border-color", "#<?php echo $border_color;?>");
					$new_or.find(".or-btn").click(cloneORCondition);
					$new_or.hide();
					$new_or.fadeIn();
					$(this).remove();
				...
				function clickEvents()
				{
					...
					$(".condition-dropdown .dropdown-menu li a").click(function(e){
						...
						$(this).each(function(){
							...
							$(this).parent().parent().parent().find("button").css("border-color", "#<?php echo $border_color;?>");
							$(this).parent().parent().parent().parent().find(".text-comparisons .text-value").attr("placeholder", "");
							if($(this).hasClass("date-fields"))
							{
								...
								$(this).parent().parent().parent().parent().find(".boolean-comparisons").hide();
							}
							else if($(this).hasClass("boolean-fields"))
							{
								//show text comparisons drop down
								$(this).parent().parent().parent().parent().find(".boolean-comparisons").show();
							...
							else
							{
								...
								if($(this).parent().parent().parent().find("button span").attr("data-selected")=="country")
								{
									$(this).parent().parent().parent().parent().find(".text-comparisons .text-value").attr("placeholder", "<?php echo _('2 letter abbreviation, eg. US')?>");
								}
					...
					//boolean drop down 'click'
					$(".boolean-comparisons .dropdown-menu li a").click(function(e){
						e.preventDefault();
						$(this).each(function(){
							$(this).parent().parent().parent().find("button span.dropdown-label").text($(this).text());
							
							if($(this).parent().parent().parent().find("button span.dropdown-label").text()=="<?php echo _('is tagged');?>")
								$(this).parent().parent().parent().find("button span.dropdown-label").attr("data-selected", "=");
							else
								$(this).parent().parent().parent().find("button span.dropdown-label").attr("data-selected", "!=");
						});
					});
						...
						$(".group").each(function(){
							if($(this).attr("id")!='group')
							{
								$(this).find(".or-group").each(function(){
									...
									{
										form_validates = true;
										$text_comparisons_input.css("border-color", "#<?php echo $border_color;?>");
										$date1.css("border-color", "#<?php echo $border_color;?>");
										$date2.css("border-color", "#<?php echo $border_color;?>");
										$con_dropdown.parent().css("border-color", "#<?php echo $border_color;?>");
									}
								});
								...
								if($(this).is(":visible"))
								{
									...
									$(this).find(".or-group").each(function(){
										if($(this).is(":visible"))
										{
											...
											else if(field_type=="boolean-fields")
											{
												comparison = $(this).find(".boolean-condition-dropdown .dropdown-label").data("selected");
												val = '1';
											}
											else
											{
				    		  ...
				    		  function(data) {
				    		      if(data)
				    		      {
					    		      ...
					    		      else
					    		      {
										  ...
										  window.location = app_path+"/includes/segments/segmentate.php?i="+app+"&l="+lid+"&s="+data+"&t="+"<?php echo get_app_info('timezone');?>"+"&r=conditions&app_path="+app_path;
									  }
				    		      }
				    		      else
				    		      {
    ...
    <div class="row-fluid">
	    ...
	    <div class="span10">    
		  ...
		  <?php 
	        ...
	        <p class="alert alert-info" style="margin-top: 20px; "><i class="icon icon-info-sign"></i> <?php echo _('When subscribers are added or removed from the list, the segmentation results will not update automatically unless you click the refresh button '); echo '<span class="icon icon-refresh"></span>.<br/>'; echo _('To have your segmentation results update automatically,')?> <a href="#cron-instructions" data-toggle="modal" style="text-decoration:underline;"><?php echo _('setup a cron job');?></a>.</p>
	         ...
	         <div id="cron-instructions" class="modal hide fade">
	            ...
	            <p><?php echo _('Once added, wait for the cron job to start running in 15 minutes. If your cron job is functioning correctly, the yellow informational message will disappear and your segmentation results will be updated automatically every 15 minutes by the cron job.');?></p>
	            </div>
	            <div class="modal-footer">
