...
$(document).ready(function() {
	...
	$("#test-form").submit(function(e){
		...
		display_errors = $form.find('input[name="display_errors"]').val(),
		url = $form.attr('action');
		org_test_send_btn_html = $("#test-send-btn").html();
		...
		if(test_email=='')
		{
			$("#test_email").effect("shake", { times:3 }, 60);
		...
		{
			$("#test-send-btn").html("<img src=\"<?php echo $loader;?>\" class=\"edit-loader\" style=\"display:block;\"/> <?php echo _('Sending');?>...");
			$("#test-send, #test-send-error, #test-send-error2").css("display", "none");
			
			$.post(url, { test_email: test_email, campaign_id: campaign_id, webversion: webversion, display_errors: display_errors },
			  function(data) {
				  ...
				  if(all_ok=='ok')
				  {
				      ...
				      $("#test-send-btn").html(org_test_send_btn_html);
				  }
				  else
				  {
	...
	$("select#email_list_exclude, select#email_list").change(function () {
	  ...
	  disable_btns();
	  $("#recipients").html("<img src='<?php echo get_app_info('path');?>/img/<?php echo get_app_info('dark_mode') ? 'loader-dark.gif' : 'loader-light.gif';?>' style='width: 15px; margin-top: -2px;'/> <?php echo _('Calculating');?>..");
	  $("#remaining").hide();
	  var inlists_array = [];
	  var exlists_array = [];
	  var inseg_array = [];
      ...
      $.post("./includes/create/calculate-totals.php", { include_lists: inlists, exclude_lists: exlists, include_lists_seg: inlists_seg, exclude_lists_seg: exlists_seg },
		  function(data) {
		      ...
		      {
				data_array = data.split("|");
				total_subs = data_array[0];
				sub_ids_json = data_array[1];				
								
				enable_btns();
			  	$("#recipients").text(total_subs);
			  	$("#remaining").show();
			  	recip = total_subs;
				...
				if($("#aws_keys_available").val()=='true' || $("#is_sub_user").val())
				{
				  ...
				  else
				  {
				      ...
				      $("#recipients").css("color", "#<?php echo get_app_info('dark_mode') ? 'FFF' : '000'?>");
				...
				//Set real data preview arrow functions
				if(total_subs != 0) //must have at least 1 subscriber
				{
					var sub_count = 1;
					campaign_id = "<?php echo encrypt_val($cid);?>";
					sub_ids = JSON.parse(sub_ids_json);
					
					$("#rdp-email").text(sub_ids.s1.email);
					$("#preview-iframe").attr("src", "<?php echo get_app_info('path')?>/w/"+sub_ids.s1.id+"/"+sub_ids.s1.list_id+"/"+campaign_id);
					...
					if(sub_count==1)
					{
						if(total_subs>1)
						{
							$("#rdp-btn-left").prop("disabled", true);
							$("#rdp-btn-right").prop("disabled", false);
						}
						else
						{
							$("#rdp-btn-left").prop("disabled", true);
							$("#rdp-btn-right").prop("disabled", true);
						}
					}
					
					$("#rdp-btn-right").unbind();
					$("#rdp-btn-right").click(function(){
						sub_count+=1;
						
						$("#rdp-email").text(sub_ids["s"+sub_count].email);
						$("#preview-iframe").attr("src", "<?php echo get_app_info('path')?>/w/"+sub_ids["s"+sub_count].id+"/"+sub_ids["s"+sub_count].list_id+"/"+campaign_id);
						...
						else if(sub_count>=100 || sub_count>=total_subs)
						{
							$("#rdp-btn-left").prop("disabled", false);
							$("#rdp-btn-right").prop("disabled", true);
						}
						else
						{
